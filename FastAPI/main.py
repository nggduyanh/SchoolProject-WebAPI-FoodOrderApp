...
async def get_user_by_account(
    user_username: str, user_password: str, db: db_dependency
):
    user = (
        db.query(models.User)
        .filter(
            models.User.username == user_username, models.User.password == user_password
        )
        .first()
    )
    if user is None:
        raise HTTPException(
            status_code=404, detail="Your Username or password is incorrect"
        )
    return user
...
async def get_customer_by_id(customer_id: int, db: db_dependency):
    customer = (
        ...
        .filter(models.Customer.id_customer == customer_id)
        .first()
    )
    if customer is None:
...
async def get_restaurant_by_id(restaurant_id: int, db: db_dependency):
    restaurant = (
        ...
        .filter(models.Restaurant.id_restaurant == restaurant_id)
        .first()
    )
    if restaurant is None:
...
async def put_restaurant(
    restaurant_id: int, restaurant: RestaurantUpdate, db: db_dependency
):
    db_restaurant = db.query(models.Restaurant).filter(
        models.Restaurant.id_restaurant == restaurant_id
    )
...
async def get_category_by_id(category_id: int, db: db_dependency):
    category = (
        ...
        .filter(models.Category.id_category == category_id)
        .first()
    )
    if not category:
